...
export default function OutreachChat({ mode = "outreach" }) {
  ...
  const copyImageToClipboard = async (base64) => {
    try {
            ...
            try {
              ...
              setMessages(prev => {
                ...
                if (data.type === "response") {
                  ...
                  // Update Active Node for Stepper
                  const nodeMatch = thoughtText.match(/^\[(\w+)\]/);
                  if (nodeMatch) {
                    let node = nodeMatch[1].toUpperCase();
                    if (node === 'SCRIBE') node = 'WRITER'; // Map Scribe to Writer
                    if (node === 'HUNTER') node = 'RETRIEVER'; // Map Hunter to Retriever
                    msg.active_node = node;
                  }
